 ...
 * 
 * Note: These tests require PostgreSQL and will be skipped in SQLite mode
 */
...
import { isPostgreSQLTest } from '../utils/db-test-utils'
import { getTestDatabaseConfig } from '../../server/database/test-config'

// Skip all tests in this file if not running PostgreSQL tests
const describeOrSkip = isPostgreSQLTest() ? describe : describe.skip

describeOrSkip('PostgreSQL Table Creation', () => {
  const config = getTestDatabaseConfig()
  const testConnectionString = config.connectionString || 'postgresql://localhost:5432/demo_data_test'
