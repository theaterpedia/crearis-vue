    ...
    <header class="list-head" :class="[`list-head--${mode}`, { 'list-head--dense': isDense }]">
        ...
        <nav class="list-head__nav">
            ...
            <template v-if="mode === 'tabs'">
                <button class="list-head__tab list-head__tab--icon"
                    :class="{ 'list-head__tab--active': activeTab === 'home' }" @click="setTab('home')"
                    aria-label="Home">
                    <Home :size="18" />
                </button>

                <button v-for="tab in entityTabs" :key="tab.id" class="list-head__tab"
                    :class="{ 'list-head__tab--active': activeTab === tab.id }" @click="setTab(tab.id)">
                    {{ tab.label }}
                </button>

                <button class="list-head__tab list-head__tab--icon"
                    :class="{ 'list-head__tab--active': activeTab === 'settings' }" @click="setTab('settings')"
                    aria-label="Settings">
                    <Settings :size="18" />
                </button>
            </template>
            ...
            <template v-else>
                ...
                <div class="list-head__dropdown-wrapper">
                    <button class="list-head__dropdown-trigger" @click="toggleDropdown" :aria-expanded="dropdownOpen">
                        <span class="list-head__dropdown-label">{{ currentTabLabel }}</span>
                        <ChevronUp v-if="dropdownOpen" :size="16" />
                        <ChevronDown v-else :size="16" />
                    </button>

                    <Transition name="dropdown">
                        <div v-if="dropdownOpen" class="list-head__dropdown-menu">
                            <button v-for="tab in entityTabs" :key="tab.id" class="list-head__dropdown-item"
...
import { Home, Search, Settings, ChevronUp, ChevronDown } from 'lucide-vue-next'
...
const currentTabLabel = computed(() => {
    ...
    const tab = entityTabs.value.find((t: ListHeadTab) => t.id === activeTab.value)
