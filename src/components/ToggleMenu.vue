<template>
  <div class="layout-toggle" ref="toggleRef">
    <button class="layout-toggle-button" @click="toggleDropdown" aria-label="Toggle layout options"
      :aria-expanded="isOpen" :class="{ 'layout-toggle-button-with-text': buttonText }">
      ...
      <span v-if="buttonText" class="layout-toggle-button-text">{{ buttonText }}</span>
    </button>
    ...
    <div v-if="isOpen" class="layout-toggle-dropdown" :class="`layout-toggle-dropdown-${placement}`">
      ...
      <template v-for="(option, index) in toggleOptions" :key="index">
        ...
        <template v-if="option.children && option.children.length > 0">
          ...
          <button v-for="(child, childIndex) in option.children" :key="`${index}-${childIndex}`"
            class="layout-toggle-option layout-toggle-option-child"
            :class="{ 'layout-toggle-option-active': modelValue === child.state }" @click="selectOption(child.state)">
            ...
            <svg v-if="modelValue === child.state" fill="currentColor" height="16" viewBox="0 0 256 256" width="16"
              xmlns="http://www.w3.org/2000/svg" class="layout-toggle-check">
              <path
        ...
        <button v-else class="layout-toggle-option"
          :class="{ 'layout-toggle-option-active': modelValue === option.state }" @click="selectOption(option.state)">
          ...
          <svg v-if="modelValue === option.state" fill="currentColor" height="16" viewBox="0 0 256 256" width="16"
            xmlns="http://www.w3.org/2000/svg" class="layout-toggle-check">
            <path
              d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z">
            </path>
      ...
      <button v-for="(option, index) in arrayOptions" :key="`array-${index}`" class="layout-toggle-setting"
...
.layout-toggle-button-with-text {
  width: auto;
  padding: 0.5rem 1rem;
}
