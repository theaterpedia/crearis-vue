<template>
  <div class="postit-thread" :class="{ expanded: isExpanded }">
    ...
    <PostITNote
      v-bind="rootComment"
      :can-edit="canEdit(rootComment)"
      :can-delete="canDelete(rootComment)"
      :can-pin="canPin"
      class="root-note"
      @edit="handleEdit"
      @delete="handleDelete"
    ...
    >
      ...
      <span v-if="!isExpanded" class="preview-avatars">
        <span
          v-for="author in previewAuthors"
          :key="author.id"
          class="preview-avatar"
          :style="{ background: getRoleColor(author.role) }"
          :title="author.name"
        >
          {{ author.initials }}
        </span>
    ...
    <Transition name="composer">
      <div v-if="showReplyComposer" class="reply-composer-wrapper">
        <PostITComposer
          :author-name="currentUser?.display_name ?? 'Du'"
