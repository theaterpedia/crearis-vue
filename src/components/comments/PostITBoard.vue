<template>
  <div class="postit-board" :class="{ loading: isLoading, empty: comments.length === 0 }">
    ...
    <div v-if="pinnedComments.length > 0" class="pinned-section">
      ...
      <div class="notes-grid pinned">
        <PostITNote
          ...
          :author-relation="comment.author.relation"
    ...
    <div class="threads-section">
      <TransitionGroup name="list" tag="div" class="notes-grid">
        <PostITNote
          ...
          :author-name="thread.parent.author.name"
          :author-relation="thread.parent.author.relation"
          :created-at="thread.parent.createdAt"
          :color="thread.parent.color"
          :is-pinned="false"
          :reactions="thread.parent.reactions"
          :reply-count="thread.totalCount - 1"
          :can-edit="canEditComment(thread.parent)"
          :can-pin="canPinComment"
          :can-delete="canDeleteComment(thread.parent)"
          @reply="handleReply(thread.parent)"
          @react="emoji => handleReact(thread.parent.id, emoji)"
          @pin="handlePin(thread.parent.id)"
          @edit="content => handleEdit(thread.parent.id, content)"
          @delete="handleDelete(thread.parent.id)"
          @showReplies="handleShowReplies(thread.parent)"
        />
      </TransitionGroup>
    </div>
    ...
    <!-- Thread view modal -->
    <BottomSheet
      v-model:is-open="showThreadView"
    ...
    >
      <template v-if="activeThread">
        ...
        <PostITNote
          ...
          :author-name="activeThread.author.name"
          :author-relation="activeThread.author.relation"
          :created-at="activeThread.createdAt"
          :color="activeThread.color"
          :is-pinned="activeThread.isPinned"
        ...
        <div class="thread-replies">
          <PostITNote
            ...
            :author-relation="reply.author.relation"
            ...
            compact
            @react="emoji => handleReact(reply.id, emoji)"
            @edit="content => handleEdit(reply.id, content)"
