<template>
    <BottomSheet :is-open="isOpen" title="Kommentare" max-height="85vh" @close="handleClose"
        ...
        <div class="mobile-toolbar">
            ...
            <button v-if="canAddComment" class="fab-btn" @click="showComposer = true">
                ＋
            </button>
        </div>
        ...
        <div class="comments-scroll">
            ...
            <div v-else-if="threads.length === 0" class="empty-state">
                ...
                <button v-if="canAddComment" class="start-btn" @click="showComposer = true">
                    Ersten schreiben
                </button>
            </div>
            ...
            <div v-if="pinnedThreads.length > 0" class="section">
                ...
                <PostITThread v-for="thread in pinnedThreads" :key="thread.root.id" :root-comment="thread.root"
                    :replies="thread.replies" :current-user-id="currentUser?.id" :current-user-role="currentUserRole"
                    :can-pin="canPin" @edit="handleEdit" @delete="handleDelete" @pin="handlePin" @reply="handleReply" />
            </div>
            ...
            <div v-if="unpinnedThreads.length > 0" class="section">
                ...
                <PostITThread v-for="thread in unpinnedThreads" :key="thread.root.id" :root-comment="thread.root"
                    :replies="thread.replies" :current-user-id="currentUser?.id" :current-user-role="currentUserRole"
                    :can-pin="canPin" @edit="handleEdit" @delete="handleDelete" @pin="handlePin" @reply="handleReply" />
            </div>
        </div>
