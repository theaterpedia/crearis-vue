<template>
    <div class="blog-page">
        <!-- Edit Panel -->
        <EditPanel v-if="project" :is-open="isEditPanelOpen" :title="`Edit ${project.heading || 'Blog'}`"
            subtitle="Update blog page information and content" :data="editPanelData" @close="closeEditPanel"
...
// SEO: Set meta tags from settings (Blog page)
function setBlogPageSeoMeta() {
    // Set title
    document.title = `Blog - ${pageSettings.seo_title}`;
    ...
    // Basic meta tags
    const blogDescription = `Neuigkeiten und Artikel aus der Theaterpedia-Community. ${pageSettings.seo_description}`;
    const blogKeywords = `${pageSettings.seo_keywords}, Blog, Artikel, Neuigkeiten`;

    setMeta('meta[name="description"]', { name: 'description', content: blogDescription });
    setMeta('meta[name="keywords"]', { name: 'keywords', content: blogKeywords });

    // Open Graph tags
    setMeta('meta[property="og:title"]', { property: 'og:title', content: `Blog - ${pageSettings.og_title}` });
    setMeta('meta[property="og:description"]', { property: 'og:description', content: blogDescription });
    ...
    setMeta('meta[name="twitter:title"]', { name: 'twitter:title', content: `Blog - ${pageSettings.og_title}` });
    setMeta('meta[name="twitter:description"]', { name: 'twitter:description', content: blogDescription });
