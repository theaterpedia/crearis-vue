...
export function useProjectStatus(project: Ref<Project | null>) {
    ...
    // Config bit checks
    const isPublic = computed(() => hasBit(project.value?.config || 0, 0))
    const isFeatured = computed(() => hasBit(project.value?.config || 0, 1))
    const registrationOpen = computed(() => hasBit(project.value?.config || 0, 2))
    const hasFunding = computed(() => hasBit(project.value?.config || 0, 3))
    const isRecurring = computed(() => hasBit(project.value?.config || 0, 4))
    const isArchivedConfig = computed(() => hasBit(project.value?.config || 0, 5))
    ...
    async function activateProject() {
        ...
        return updateProjectStatus({
            status: 0x04, // Active
            config: setBit(project.value!.config || 0, 0) // Set public
        })
    }
    ...
    async function completeProject(summary?: string) {
        ...
        return updateProjectStatus({
            status: 0x08, // Completed
            config: clearBit(project.value!.config || 0, 2) // Close registration
        })
    }
    ...
    async function archiveProject() {
        ...
        return updateProjectStatus({
            status: 0x10, // Archived
            config: clearBit(clearBit(project.value!.config || 0, 0), 1) // Clear public and featured
        })
    }
    ...
    async function setConfigBit(bit: number, value: boolean) {
        ...
        const newConfig = value
            ? setBit(project.value.config || 0, bit)
            : clearBit(project.value.config || 0, bit)
    ...
    async function toggleConfigBit(bit: number) {
        ...
        return updateProjectStatus({
            config: toggleBit(project.value.config || 0, bit)
        })
    }
