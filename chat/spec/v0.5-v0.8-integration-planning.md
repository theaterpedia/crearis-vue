...
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                     CREARIS-ODOO (Backend)                             │ │
│  │  ════════════════════════════════════════════════════════════════════  │ │
│  │                                                                        │ │
│  │  • Odoo 16 Community Edition (upgradeable to 18)                       │ │
│  │  • Multi-company setup with domaincode-indexed websites                │ │
│  │  • Custom extensions:                                                  │ │
│  │    - domainuser table (user ↔ website relations + roles)               │ │
│  │    - md fields on all entities (Markdown support)                      │ │
│  │    - Heading/Hero component configuration                              │ │
│  │    - Extended events module (beyond "selling conferences")             │ │
│  │  • GraphQL API with xmlid-based everything                             │ │
│  │  • Workitems/activities system (untouched, ready to integrate)         │ │
│  │                                                                        │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                     │                                        │
│                    ┌────────────────┼────────────────┐                       │
...
│                    ▼                ▼                ▼                       │
│         ┌─────────────────┐  ┌─────────────┐  ┌─────────────────┐           │
│         │   MCP Server    │  │  GraphQL    │  │  JSON-RPC       │           │
│         │   (new option)  │  │  (existing) │  │  (standard)     │           │
│         └────────┬────────┘  └──────┬──────┘  └────────┬────────┘           │
│                  │                  │                  │                     │
│                  └──────────────────┼──────────────────┘                     │
│                                     │                                        │
│                                     ▼                                        │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                     CREARIS-VUE (Current Focus)                        │ │
│  │  ════════════════════════════════════════════════════════════════════  │ │
│  │                                                                        │ │
│  │  • Simplified practitioner UI (tablet-optimized, onboarding-first)     │ │
│  │  • Sandboxed Odoo functionality                                        │ │
│  │  • /sites/[domaincode] preview routes                                  │ │
│  │  • Nitro backend + Vue frontend                                        │ │
│  │  • PostgreSQL (simplified Odoo schema cache)                           │ │
│  │  • sysreg_config capabilities system                                   │ │
│  │                                                                        │ │
│  │  Integration points:                                                   │ │
│  │  • nitro-graphql → Crearis-Odoo GraphQL                               │ │
│  │  • workitems table → Odoo activities sync                              │ │
│  │  • projects table → Odoo companies/websites                            │ │
│  │                                                                        │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                     │                                        │
│                                     ▼ (v1.0, April 2026)                     │
...
│  │  ════════════════════════════════════════════════════════════════════  │ │
│  │                                                                        │ │
│  │  • Standalone websites with real domains                               │ │
│  │  • utopia-in-action.de, other-domain.org                               │ │
│  │  • Deep Odoo integration (payments, etc.)                              │ │
│  │  • Components inherited from Crearis-Vue                               │ │
...
3. **Multi-Odoo future:** Multiple Odoo instances → single Vue experience
...
- Unified GraphQL layer (Vue ↔ Odoo)
...
```
┌─────────────────┐                    ┌─────────────────┐
│ Claude @ Vue    │                    │ Claude @ Odoo   │
│ (this session)  │                    │ (other session) │
├─────────────────┤                    ├─────────────────┤
│                 │                    │                 │
│  1. Create      │   ──(report)──▶    │  2. Read        │
│     spec.md     │                    │     spec.md     │
│                 │                    │                 │
│  3. Query       │   ◀──(MCP)────     │  4. Create      │
│     results     │                    │     test data   │
│                 │                    │                 │
│  5. Verify      │   ──(MCP)────▶     │  6. Check       │
│     sync        │                    │     Odoo state  │
│                 │                    │                 │
└─────────────────┘                    └─────────────────┘
```
...
- [ ] `mail.activity` ↔ workitems sync
- [ ] `res.partner` ↔ users sync verification
- [ ] `website` ↔ projects sync verification
...
```
┌─────────────────────────────────────────────────────────────────┐
│                    WORKITEMS UNIFICATION                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Source 1: Odoo Business Processes                               │
│  ├─ mail.activity (scheduled activities, reminders)              │
│  ├─ project.task (kanban tasks, project work)                    │
│  └─ mail.message (discussions, linked to activities)             │
│                                                                  │
│  Source 2: Comments (Migration 057)                              │
│  ├─ Post-IT style discussions                                    │
│  ├─ Threaded replies                                             │
│  └─ Entity-attached (post, project, event, image)                │
│                                                                  │
│  Source 3: Admin/Poweruser Scripts                               │
│  ├─ Data import/export tasks                                     │
│  ├─ Domain configuration                                         │
│  └─ System administration                                        │
│                                                                  │
│  Source 4: Existing Kanban Tasks                                 │
│  ├─ Entity-aligned task cards                                    │
│  ├─ Status-value system integration                              │
│  └─ Fullscreen kanban visualization                              │
│                                                                  │
│                          ▼                                       │
│            ┌─────────────────────────┐                           │
│            │      workitems          │                           │
...
<template>
  ...
  <div v-else>
    ...
    <div class="stub-preview">
      🚧 Workitems panel coming in v0.5
    </div>
  </div>
</template>
...
- **tuanle96/mcp-odoo:** https://github.com/tuanle96/mcp-odoo (235 ⭐)
